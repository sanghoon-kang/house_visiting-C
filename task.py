...
class Task:
    def __init__(self,window,drawer):
        ...
        self.amountStim = visual.TextStim(self.window, 
                            height = 50,
                            pos=(0, 0), text='+10원',
                            bold=True,
                            anchorHoriz = 'center',
    ...
    def instrumental_component(self, step, alphabetornumber, wordStim, captured_string, instruction,
        ...
        if step == 'instruction':
            ...
            if paired_color in insideStim:
                ...
                if c == 3:
                    if instrt<=2.5:
                        money = round(get_truncated_normal(mean=60, sd=5, low=40, upp=80, n=1)[0]+(1.5-instrt)*40).astype(int)
            ...
            if paired_color in insideStim:
                ...
                if c == 3:
                    if instrt<=2.5:
                        money = np.abs(round(((1.5-instrt)*40)-get_truncated_normal(mean=60, sd=5, low=40, upp=80, n=1)[0]).astype(int))
                        moneyStim = visual.TextStim(win,
                                    height=50,
                                    pos=(0,-50), text = ("-%s원" %str(money)),
                                    bold=True,
                        ...
                        money = round(get_truncated_normal(mean=60, sd=5, low=40, upp=80, n=1)[0]).astype(int)
                        moneyStim = visual.TextStim(win,
                                    height=50,
                                    pos=(0,-50), text = ("-%s원" %str(money)),
                                    bold=True,
...
#### face liking task set-up ####
    def face_liking_task(self, txt_color=(1,1,1), day=1):
        ...
        drawer=self.drawer
        if day==1:
            text = update_text(self.text,instructions['faces_liking_task'])
        else:
...
#### movie set-up ####
    def movie(self):
        win = self.window
        drawer = self.drawer
        movie = visual.MovieStim3(win, '../Desktop/NZ/NZ.mp4', size = (1920,1080), opacity=1.0, pos = (0,0))
...
class Runner:
    ...
    def day1(self):
        ...
        while n<5:
            ...
            task.instrumental_component(step='instruction', wordStim=wordStim, captured_string=captured_string, instruction = instruction, paired_color=paired_color,
                                         insideStim='template.png', alphabetornumber=number, day=None)
        ...
        while block<1:
            n=0
            while n<9:
                insideStim = visual.ImageStim(task.window, image = path['inside'] + 'night_inside/' + house_list[n], size = (1920,1080), pos = (0,0))
                winsoundStim = sound.Sound(path['winsound'])
                pmoney = round(get_truncated_normal(mean=10, sd=2, low=5, upp=20, n=1)[0]).astype(int)
                p1money = round(get_truncated_normal(mean=0, sd=0.5, low=0, upp=2, n=1)[0]).astype(int)
                ...
                if paired_color in house_list[n]:
                    ...
                    if p1money ==0:
                        ...
                        amount1Stim.draw()
                        task.window.flip()
                        winsoundStim.play() #should there be sound for this? Since it's actually not CS+...
                        core.wait(2)
                    pass
                    pass
    ...
    def day2(self):
        ...
        if param["condition"] == 'pR':
            ...
            while block<3:
                ...
                while n<2:  # Use two of the stimuli
                    ...
                    insideStim = visual.ImageStim(win, image = path['inside'] + 'night_inside/' + paired_list[n], size = (1920,1080), pos = (0,0))
                    pmoney = round(get_truncated_normal(mean=10, sd=2, low=5, upp=20, n=1)[0]).astype(int)
                    ...
                    insideStim.draw()
                    task.plusStim.draw()
                    task.amountStim.draw()
                    win.flip()
                    task.winsoundStim.play()
                    core.wait(2)
                    ...
                    task.instrumental_component(step='conditioning', wordStim=wordStim, captured_string=captured_string, instruction=instruction,
                                                insideStim=paired_list[n], paired_color=paired_color, alphabetornumber=alphabet, day=1, df1=df1)
        ...
        drawer.draw_text(text='연구자의 지시를 따라 주시기 바랍니다.', pos = (0,0), height = 50, wrap=2000, day=2)
        core.wait(1)
        win.flip()
        _=event.waitKeys(keyList=["space"])

        #Counterconditioning
        text = update_pos(update_text(task.text, instructions['day2_house'], day=2), (0,0))
        text.draw()
        drawer.draw_spacebar(day=2)
        core.wait(1)
        win.flip()
        ...
        while block<1:
            ...
            while n<9:
                ...
                captured_string, df1 = task.instrumental_component(step='counterconditioning', wordStim=wordStim, captured_string=captured_string, instruction=instruction,
                                                                    insideStim=house_list[n], paired_color=paired_color, alphabetornumber=alphabet, day=2, df1=df1)
